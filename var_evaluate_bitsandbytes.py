...
# def replace_linear_with_bnb_fp4(model):
#     for name, module in model.named_children():
#         if isinstance(module, torch.nn.Linear):
#             new_module = bnb_nn.Linear4bit(
#                 module.in_features,
#                 module.out_features,
...
#             with torch.no_grad():
#                 new_module.weight = bnb_nn.Params4bit(module.weight.data, requires_grad=False, compress_statistics=True, quant_type='nf4').cuda()
#                 if module.bias is not None:
#                     new_module.bias = module.bias
#             setattr(model, name, new_module.cuda())
#         else:
#             replace_linear_with_bnb_fp4(module)
def quant_all_layers(model, groupsize=128, name_filter=None):
    """Recursively quantize all `torch.nn.Linear` layers in the model.
    @model: top-level model in which to perform the quantization
    @groupsize: marlin groupsize
    @name_filter: lambda indicating if a layer should be quantized
    """
    for attr in dir(model):
        tmp = getattr(model, attr)
        
        if isinstance(tmp, torch.nn.Linear):
            print(f'Quantizing {attr}')
            # reshape the weight to fit the quantization
            tmp_weight_data = tmp.weight.data.view(-1, tmp.weight.data.shape[-1])
            scale = torch.max(torch.abs(tmp_weight_data), dim=0, keepdim=True)[0] * 2 / (2 ** 4 - 1)
            tmp_weight_data = torch.clamp(torch.round(tmp_weight_data / scale), min=-8, max=7) * scale
            tmp.weight.data = tmp_weight_data.view(tmp.weight.data.shape)
            
    for name1, child in model.named_children():
        quant_all_layers(child, groupsize=groupsize, name_filter=name_filter)
...
def main():
    ...
    quant_all_layers(var.blocks)
    print(var)
    
    # marlin.replace_linear(
    #     var,
    #     name_filter=lambda n: 'block' in n,
    #     groupsize=128
    # )
    ...
    # generate images
    output_dir = f'var_d{args.model_depth}_cfg{args.cfg}_seed{args.seed}_bitsandbytes'
