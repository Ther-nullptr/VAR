"""
Enhanced VAR model with stage-specific layer skipping functionality.
Allows skipping transformer layers based on layer index and generation stage.
"""
...
class LayerSkipConfig:
    """Configuration class for layer skipping"""
    def __init__(
        self,
        stage_layer_skip: Dict[int, Set[int]] = None,  # {stage_idx: {layer_indices_to_skip}}
        ...
        """
        Args:
            stage_layer_skip: Dictionary mapping stage index to set of layer indices to skip
            skip_all_layers_below: Dictionary mapping stage index to minimum layer index (skip all below)
            skip_all_layers_above: Dictionary mapping stage index to maximum layer index (skip all above)
            enable_skip: Global enable/disable for layer skipping
        
        Examples:
            # Skip layers 0,1,2 in the last stage (stage 9 for patch_nums=(1,2,3,4,5,6,8,10,13,16))
            LayerSkipConfig(skip_all_layers_below={9: 3})
            
            # Skip specific layers in specific stages
            LayerSkipConfig(stage_layer_skip={8: {0, 1, 15}, 9: {0, 1, 2}})
            
            # Skip all layers with index < 1 in the last stage
            LayerSkipConfig(skip_all_layers_below={9: 1})
        """
    ...
    def should_skip_layer(self, stage_idx: int, layer_idx: int) -> bool:
        """Check if a layer should be skipped in a given stage"""
    ...
    def get_skip_summary(self, num_stages: int, num_layers: int) -> str:
        """Get a summary string of the skip configuration"""
        if not self.enable_skip:
            return "Layer skipping disabled"
        
        summary = []
        for stage_idx in range(num_stages):
            skipped_layers = []
            for layer_idx in range(num_layers):
                if self.should_skip_layer(stage_idx, layer_idx):
                    skipped_layers.append(layer_idx)
            
            if skipped_layers:
                summary.append(f"Stage {stage_idx}: skip layers {skipped_layers}")
        
        return "; ".join(summary) if summary else "No layers skipped"
...
class VARLayerSkip(nn.Module, PyTorchModelHubMixin):
    """
    VAR model with stage-specific layer skipping functionality
    """
    def __init__(
        ...
        layer_skip_config: LayerSkipConfig = None,
        ...
        self.layer_skip_config = layer_skip_config or LayerSkipConfig()
        ...
        print(f'[VARLayerSkip] Layer skipping config: {self.layer_skip_config.get_skip_summary(len(self.patch_nums), depth)}')
    ...
    def autoregressive_infer_cfg(
        ...
        """
        Autoregressive inference with stage-specific layer skipping
        """
        ...
        # Track skipped layers for logging
        ...
        for si, pn in enumerate(self.patch_nums):   # si: i-th segment (stage)
            ...
            # Apply transformer blocks with layer skipping
            layers_skipped_this_stage = 0
            for layer_idx, block in enumerate(self.blocks):
                total_layers_processed += 1
                
                # Check if this layer should be skipped in this stage
                if self.layer_skip_config.should_skip_layer(si, layer_idx):
                    layers_skipped_this_stage += 1
                    total_layers_skipped += 1
                    # Skip this layer - x remains unchanged
                    continue
            ...
            if layers_skipped_this_stage > 0:
                print(f"Stage {si} (patch_size={pn}x{pn}): skipped {layers_skipped_this_stage}/{len(self.blocks)} layers")
        ...
        skip_ratio = total_layers_skipped / total_layers_processed if total_layers_processed > 0 else 0
        print(f"Total layer skipping: {total_layers_skipped}/{total_layers_processed} ({skip_ratio:.1%})")
...
def create_var_layer_skip_model(
    vae_local: VQVAE,
    layer_skip_config: LayerSkipConfig = None,
    **kwargs
) -> VARLayerSkip:
    """
    Factory function to create VARLayerSkip model with specified layer skipping configuration
    """
    return VARLayerSkip(
        vae_local=vae_local,
