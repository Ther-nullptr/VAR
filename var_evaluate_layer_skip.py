"""
VAR evaluation script with stage-specific layer skipping functionality
"""
...
def parse_stage_layer_skip(arg_str: str) -> dict:
    """
    Parse stage-layer skip configuration from string
    Format: "stage1:layer1,layer2;stage2:layer3,layer4"
    Example: "9:0,1,2" means skip layers 0,1,2 in stage 9
    """
    if not arg_str:
        return {}
...
def parse_stage_threshold_skip(arg_str: str) -> dict:
    """
    Parse stage-threshold skip configuration from string
    Format: "stage1:threshold1;stage2:threshold2"
    Example: "9:3" means skip all layers with index < 3 in stage 9
    """
    if not arg_str:
        return {}
...
def build_vae_var_layer_skip(
    ...
    # Layer skipping args
    layer_skip_config: Optional[LayerSkipConfig] = None
) -> Tuple[VQVAE, VARLayerSkip]:
    """
    Build VAE and VAR model with layer skipping functionality
    """
    heads = depth
    width = depth * 64
    dpr = 0.1 * depth/24
    ...
    # build VAR with layer skipping
    var_wo_ddp = VARLayerSkip(
        vae_local=vae_local,
        ...
        layer_skip_config=layer_skip_config
    ).to(device)
...
def parse_args():
    parser = argparse.ArgumentParser(description='VAR model evaluation with stage-specific layer skipping')
    ...
    parser.add_argument('--stage_layer_skip', type=str, default='',
                        help='Stage-specific layer skipping. Format: "stage1:layer1,layer2;stage2:layer3,layer4". Example: "9:0,1,2" skips layers 0,1,2 in stage 9')
    parser.add_argument('--skip_layers_below', type=str, default='',
                        help='Skip all layers below threshold in specific stages. Format: "stage1:threshold1;stage2:threshold2". Example: "9:3" skips layers 0,1,2 in stage 9')
    parser.add_argument('--skip_layers_above', type=str, default='',
                        help='Skip all layers above threshold in specific stages. Format: "stage1:threshold1;stage2:threshold2". Example: "9:13" skips layers 14,15 in stage 9')
    parser.add_argument('--disable_layer_skip', action='store_true',
                        help='Disable all layer skipping')
...
def create_layer_skip_config_from_args(args) -> LayerSkipConfig:
    """Create LayerSkipConfig from command line arguments"""
    ...
    return LayerSkipConfig(
        ...
        enable_skip=not args.disable_layer_skip
    )


def generate_output_dir_name(base_dir: str, args, layer_skip_config: LayerSkipConfig) -> str:
    """Generate output directory name that includes layer skipping parameters"""
    components = []
    ...
    # Layer skipping configuration
    if layer_skip_config.enable_skip:
        if layer_skip_config.stage_layer_skip:
            skip_str = "_".join([f"s{k}l{'_'.join(map(str, sorted(v)))}" 
                               for k, v in layer_skip_config.stage_layer_skip.items()])
            components.append(f"skip_{skip_str}")
        
        if layer_skip_config.skip_all_layers_below:
            below_str = "_".join([f"s{k}below{v}" 
                                for k, v in layer_skip_config.skip_all_layers_below.items()])
            components.append(f"skipbelow_{below_str}")
        
        if layer_skip_config.skip_all_layers_above:
            above_str = "_".join([f"s{k}above{v}" 
                                for k, v in layer_skip_config.skip_all_layers_above.items()])
            components.append(f"skipabove_{above_str}")
    else:
        components.append("no_layer_skip")
    ...
    dir_name = "var_layer_skip_" + "_".join(components)
...
def main():
    ...
    layer_skip_config = create_layer_skip_config_from_args(args)
    
    # Generate descriptive output directory name
    base_output_dir = args.output_dir
    param_output_dir = generate_output_dir_name(base_output_dir, args, layer_skip_config)
    ...
    print(f"\nLayer Skipping Configuration:")
    print(f"{'='*60}")
    print(layer_skip_config.get_skip_summary(10, args.model_depth))  # 10 stages for default patch_nums
    print(f"{'='*60}\n")
    ...
    print("Building VAR model with layer skipping...")
    ...
    if results:
        ...
        results['parameters'] = {
            ...
            'more_smooth': args.more_smooth,
            'layer_skip_config': {
                'stage_layer_skip': layer_skip_config.stage_layer_skip,
                'skip_all_layers_below': layer_skip_config.skip_all_layers_below,
                'skip_all_layers_above': layer_skip_config.skip_all_layers_above,
                'enable_skip': layer_skip_config.enable_skip,
                'skip_summary': layer_skip_config.get_skip_summary(10, args.model_depth)
            }
        }
        
        results_path = os.path.join(args.output_dir, 'layer_skip_results.json')
    ...
    print("\nVAR layer skipping evaluation completed!")
