...
def quant_all_layers(model, groupsize=128, name_filter=None):
    ...
    for attr in dir(model):
        ...
        if isinstance(tmp, torch.nn.Linear):
            ...
            tmp_weight_data = tmp.weight.data.view(-1, tmp.weight.data.numel() // groupsize)
...
def main():
    ...
    quant_all_layers(
        var,
        groupsize=128,
        name_filter=lambda n: 'blocks' in n  # Only quantize attention layers
    )
    ...
    # generate images
    output_dir = f'var_d{args.model_depth}_cfg{args.cfg}_seed{args.seed}_fake'
